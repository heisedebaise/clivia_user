...
class _DestroyPasswordState extends State<DestroyPassword> {
  ...
  Widget build(BuildContext context) => SwitchListTile(
        title: Text(l10n(null, 'sign.destroy')),
        subtitle: Text(l10n(null, 'sign.destroy.memo')),
        value: User.destroy(),
        onChanged: (bool on) {
          PageRouter.push(
            PasswordPage(
              l10n(null, 'sign.destroy.${on ? 'on' : 'off'}'),
              'user.sign-in.destroy',
              twice: on,
              full: true,
              complete: (value) async {
                return await User.passwordOnOff(context, on, 'destroy', value, setState, l10n(null, 'sign.destroy.wrong'));
              },
            ),
          );
        },
...
class Destroy extends StatelessWidget {
  ...
  Widget build(BuildContext context) => ListTile(
        title: Text(l10n(null, 'sign.destroy.account')),
        subtitle: Text(l10n(null, 'sign.destroy.account.memo')),
          ...
          showDialog(
            ...
            builder: (context) => AlertDialog(
              ...
              content: Column(
                ...
                children: [
                  Text(l10n(null, 'sign.destroy.account.memo')),
                  TextField(
                    obscureText: true,
                    decoration: InputDecoration(
                      prefixIcon: const Icon(Icons.lock),
                      labelText: l10n(null, 'sign.destroy.password'),
                    ),
              ...
              actions: [
                ...
                TextButton(
                    onPressed: () async {
                      ...
                      if (data != null && !data) {
                        Notice.error(0, l10n(null, 'sign.password.wrong'));
                      }
                    },
                    child: Text(
                      l10n(null, 'ok'),
